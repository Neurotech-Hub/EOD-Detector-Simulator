...
| **C5** | 2.2 nF | **remove** (or ≤ 470 pF) | 2.2 nF directly loads the INA333 output and forms an under-damped resonance; the front end self-oscillates and TRIGGER chatters continuously — the "stuck on" signature. **Bench-confirmed:** ~80 kHz, ~220 mV pk-pk ring at ELEC_OUT, eliminated by removing C5. Detection is identical without it (C4 + amp bandwidth already band-limit the path); 470 pF C0G is acceptable as EMI insurance. Details: [C4_STABILITY.md](C4_STABILITY.md). |
